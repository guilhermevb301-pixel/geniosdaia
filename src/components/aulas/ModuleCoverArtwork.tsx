...
const REFLECTIONS: Record<
  ModuleCoverReflection,
  { line: string; wash: string }
> = {
  emerald: {
    line: "bg-primary/20",
    wash: "linear-gradient(118deg, hsl(var(--primary) / 0.08) 0%, transparent 24%)",
  },
  codex: {
    line: "bg-codex/20",
    wash: "linear-gradient(118deg, hsl(var(--codex) / 0.08) 0%, transparent 24%)",
  },
  claude: {
    line: "bg-claude/20",
    wash: "linear-gradient(118deg, hsl(var(--claude) / 0.08) 0%, transparent 24%)",
  },
};
...
function Motif({ layout, focus }: MotifProps) {
  switch (layout) {
    case "portal":
      return (
        <>
          <span className={`absolute left-[16%] top-[15%] h-[70%] w-[68%] rounded-full border ${focus.border}`} />
      ...
      return (
        <>
          ...
          {[18, 38, 58, 78].map((left, index) => (
            <span
              key={left}
              className={`absolute bottom-[21%] h-2 border ${index === 2 ? focus.border : "border-white/10"}`}
      ...
      return (
        <>
          ...
          {[
            ...
            <span
              key={`${left}-${top}`}
              className={`absolute h-3 w-3 rounded-full border ${index === 2 ? focus.border : "border-white/20"} bg-[#11161b]`}
              style={{ left, top }}
            />
          ))}
        </>
    ...
    case "funnel":
      return (
        <>
          <span
            className={`absolute left-[18%] top-[18%] h-[20%] w-[64%] border ${focus.border}`}
          ...
          />
          <span className={`absolute bottom-[18%] left-[46%] h-[16%] w-[8%] ${focus.fill} opacity-50`} />
        </>
      );
    case "orbit":
      return (
        <>
          <span className={`absolute left-[12%] top-[31%] h-[38%] w-[76%] rotate-[-12deg] rounded-full border ${focus.border}`} />
          <span className="absolute left-[24%] top-[17%] h-[66%] w-[52%] rotate-[28deg] rounded-full border border-white/10" />
          <span className={`absolute right-[15%] top-[30%] h-3 w-3 rounded-full ${focus.fill}`} />
        </>
      );
    case "vault":
      return (
        <>
          <span className={`absolute left-[20%] top-[17%] h-[66%] w-[60%] rounded-md border-2 ${focus.border}`} />
          <span className="absolute left-[27%] top-[26%] h-[48%] w-[46%] rounded-full border border-white/20" />
          <span className={`absolute left-[28%] top-1/2 h-px w-[44%] ${focus.fill} opacity-40`} />
        </>
      );
    case "burst":
      return (
        <>
          <span className={`absolute left-[31%] top-[24%] h-[52%] w-[38%] rotate-45 border ${focus.border}`} />
      ...
      return (
        <>
          {[28, 46, 64].map((size, index) => (
            <span
              key={size}
              className={`absolute left-1/2 top-1/2 rounded-full border ${index === 1 ? focus.border : "border-white/10"}`}
              style={{
                height: `${size}%`,
                width: `${size}%`,
      ...
      return (
        <div className="absolute inset-x-[15%] bottom-[16%] flex h-[54%] items-end justify-between gap-2">
          {[26, 48, 34, 76, 58, 92].map((height, index) => (
            <span
              key={`${height}-${index}`}
              className={index === 3 ? focus.fill : "bg-white/10"}
    ...
    case "radar":
      return (
        <>
          ...
          <span className={`absolute left-1/2 top-1/2 h-px w-[32%] origin-left -rotate-[32deg] ${focus.fill} opacity-60`} />
        </>
      );
    case "stage":
      return (
        <>
          <span
            className={`absolute left-[28%] top-[-8%] h-[82%] w-[44%] ${focus.soft} blur-xl`}
    ...
    case "portrait":
      return (
        <>
          <span className={`absolute left-[29%] top-[12%] h-[76%] w-[42%] rounded-[45%] border ${focus.border}`} />
          <span className="absolute left-[23%] top-[20%] h-[60%] w-[54%] rounded-[42%] border border-white/10" />
          <span className={`absolute bottom-[12%] left-[42%] h-1 w-[16%] rounded-full ${focus.fill} opacity-50`} />
        </>
      );
    case "stack":
      return (
        <>
          {[0, 1, 2].map((index) => (
            <span
              key={index}
              className={`absolute h-[48%] w-[48%] rounded-md border ${index === 2 ? focus.border : "border-white/10"} bg-[#11161b]/80`}
    ...
    case "frame":
      return (
        <>
          <span className={`absolute left-[15%] top-[15%] h-[70%] w-[70%] rounded-md border ${focus.border}`} />
          <span className="absolute left-[22%] top-[23%] h-[54%] w-[56%] border border-dashed border-white/20" />
          <span className={`absolute right-[11%] top-[11%] h-2 w-2 rounded-full ${focus.fill}`} />
        </>
      );
    case "split":
      return (
    ...
    case "market":
      return (
        <>
          ...
          <span className={`absolute left-[17%] top-[30%] h-px w-[68%] -rotate-[18deg] ${focus.fill} opacity-40`} />
        </>
      );
    case "studio":
      return (
        <>
          {[32, 48, 64].map((size, index) => (
            <span
              key={size}
              className={`absolute left-1/2 top-1/2 rounded-full border ${index === 0 ? focus.border : "border-white/10"}`}
              style={{
                height: `${size}%`,
                width: `${size}%`,
      ...
      return (
        <div className="absolute inset-x-[12%] top-1/2 flex h-[46%] -translate-y-1/2 items-center justify-between gap-1.5">
          {[18, 42, 76, 48, 92, 62, 32, 54, 22].map((height, index) => (
            <span
              key={`${height}-${index}`}
              className={index === 4 ? focus.fill : "bg-white/20"}
    ...
    case "growth":
      return (
        <>
          ...
          <span className={`absolute left-[20%] top-[44%] h-px w-[60%] -rotate-[24deg] ${focus.fill} opacity-50`} />
        </>
      );
    case "browser":
      return (
        <>
          <span className={`absolute left-[12%] top-[16%] h-[68%] w-[76%] rounded-md border ${focus.border}`} />
...
export function ModuleCoverArtwork({
  ...
  const reflection = REFLECTIONS[metadata.reflection];
  ...
  return (
    <div
      ...
      className={`relative isolate h-full w-full overflow-hidden bg-[#0b0d10] ${
        locked
          ? "grayscale opacity-60"
          : "motion-safe:transition-transform motion-safe:duration-300 group-hover/card:scale-[1.025]"
      }`}
      style={{
        backgroundImage: `${reflection.wash}, linear-gradient(145deg, #151a20 0%, #090b0e 72%)`,
      }}
    >
      <span
        className={`absolute left-[8%] top-0 h-px w-[58%] rotate-[18deg] ${reflection.line}`}
      />
      <span className="absolute bottom-[6%] right-[5%] h-px w-[48%] -rotate-[20deg] bg-primary/15" />
      <span className="absolute inset-[7%] rounded-[6px] border border-white/[0.06]" />

      <Motif layout={metadata.layout} focus={FOCUS} />

      {[0, 1, 2].map((index) => (
        <span
          key={index}
          className={`absolute h-1.5 w-1.5 rounded-full ${index === 1 ? FOCUS.fill : "bg-white/20"}`}
          style={markerStyle(index)}
        />
      ))}

      <span
        className={`absolute left-1/2 top-1/2 flex h-[34%] w-[34%] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[8px] border bg-[#11161b]/90 ${FOCUS.border} ${FOCUS.text}`}
